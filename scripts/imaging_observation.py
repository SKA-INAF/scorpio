...
def get_args():
	...
	parser.add_argument('-field', '--field', dest='field', required=False, type=int, default=-1,action='store',help='Field to be processed (default=-1 (all fields))')
...
def main():
	...
	## Image each pointing if field_imaged=-1, otherwise selected pointing
	t_start = time.time()
	...
	if field_imaged == -1:
		...
		print ("INFO: Cleaning selected field ...")
		
		## Check selected field
		field= field_imaged
		if field<0 or field>=npointings:
			print ("ERROR: Invalid field %s selected (hint: valid fields are in range [0-%s]" % (0,npointings-1))
			return 1
